...
QString Syncer::syncNow(bool upload_only)
{
	...
	error = syncDirectory(last_sync_time, DatUtil::getCurrentFolder(), "cdata/", DataAccess::currentData, server_list, Character::instance->getCurrentDataFile(), upload_only);
	if (!error.isEmpty())
		return error;
